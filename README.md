...
3. Передайте ключи через окружение (рекомендуется; в git не коммитится):
   ```bash
   export TELEGRAM_BOT_TOKEN="123456:ABC-DEF..."
   export TELEGRAM_CHAT_ID="42"
   ```
   либо заполните поля `telegram_bot_token` / `telegram_chat_id` в
   `config/live.yaml` (конфиг приоритетнее окружения). В docker-деплое ключи
   кладутся в файл `.env` рядом с `docker-compose.yml` (см. пример ниже).
   Значения никогда не попадают в логи — логируется только сам факт
   enabled/disabled.
...
| --- | --- | --- |
| 🟢 «Раннер запущен» | старт процесса (один раз), с позицией на момент старта | нет |
...
| ⚠️ network error | сетевые/данные сбои цикла | не чаще раза в `error_throttle_minutes` |
| 💤 Heartbeat | дайджест «позиция/equity/возраст свечи/свитчи» | раз в `heartbeat_hours` (0 — выключить) |
