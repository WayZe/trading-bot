...
1. Если позиция открыта — тикер проверяется на пробой активных стоп/тейк
   ...
   по фактической цене, как консервативно и в бэктесте).
2. Догружаются закрытые свечи (та же семантика «свеча закрыта ⟺
   ...
   стратегии; сигналы исполняются сразу.
3. Уровни стоп/тейк входного сигнала переякориваются на фактическую цену
   входа **теми же функциями**, что и в бэктест-движке (`engine/stops.py`) —
   parity-тест гарантирует, что live-стоп равен бэктест-стопу на том же
   сценарии.
4. Состояние (позиция, стопы, последняя обработанная свеча, equity paper,
   сделки) пишется в `data/live/state.json` после каждой свечи — рестарт
   посреди батча не теряет историю. Стратегия после рестарта прогревается
   на истории и «вспоминает» открытую позицию.
...
### Ошибки и `needs_attention`

Сетевые сбои не убивают раннер (warning и повтор на следующем тике). Если
же ордер в testnet не дошёл или его статус неизвестен, состояние позиции
неясно: раннер ставит флаг `needs_attention` в state и **останавливает
торговлю** до ручного разбора (heartbeat в логе кричит об этом каждый
цикл). Флаг снимается правкой `state.json` вручную. В testnet при старте
позиция сверяется с балансом биржи (`reconcile`): по количеству доверяем
бирже, расхождение логируется.
