...
- `--once` — один цикл и выход (cron-дружелюбно; повторный запуск не
  дублирует свечи). Стартовое Telegram-сообщение и heartbeat-дайджест в этом
  режиме отключены (см. «Уведомления Telegram»); сделки, свитчи, сбои и
  `needs_attention` уведомляются как раньше.
...
Параллельный запуск над тем же state-файлом запрещён: команда `live`
берёт эксклюзивную блокировку на `<state>.lock` (`fcntl.flock`) и при
занятом локе отказывается стартовать («already running») — два раннера
на одном состоянии задвоили бы ордера.

### Свитчи: STOP и PAUSE

Два независимых файла-выключателя (пути настраиваются, при старте
проверяется, что они не совпадают друг с другом и с state-файлом):

- **STOP** (`data/live/STOP`) — аварийный полный стоп: пока файл существует,
  цикл продолжает работать (тикеры, свечи, логи), но **любые новые ордера
  запрещены** — включая защитные выходы по стопу/тейку. Открытая позиция
  остаётся без присмотра, поэтому каждый цикл раннер пишет error с ценой
  тикера и дистанцией до стопа. Удалите файл, чтобы возобновить торговлю.
- **PAUSE** (`data/live/PAUSE`) — «мягкая» остановка: запрещены **только
  новые входы**; сигнальные выходы и защитные стоп/тейк исполняются как
  обычно. Удобно, когда стратегию нужно заморозить, но открытую позицию —
  довести до выхода по плану.

Оба флага видны в баннере при старте и в heartbeat каждого цикла
(`kill_switch=... pause=...`).
...
Раннер умеет слать ключевые события в Telegram — это «пейджер» на случай,
если что-то пойдёт не так, пока вас нет у терминала. Уведомления опциональны:
без настроенных ключей раннер работает как раньше (в логе один раз пишется
«notifications are disabled»).

Как включить:

1. Создайте бота у [@BotFather](https://t.me/BotFather) (команда `/newbot`)
   и сохраните токен вида `123456:ABC-DEF...`.
2. Напишите своему боту любое сообщение (например, «привет») — боты не шлют
   первым. Затем откройте `https://api.telegram.org/bot<ТОКЕН>/getUpdates`
   и возьмите `chat.id` из ответа (для канала добавьте бота администратором
   и используйте id канала вида `@channelname` или `-100...`).
3. Передайте ключи через переменные окружения — это единственный
   поддерживаемый способ, `config/live.yaml` в git и секретов в нём нет:
   ```bash
   export TELEGRAM_BOT_TOKEN="123456:ABC-DEF..."
   export TELEGRAM_CHAT_ID="42"
   ```
   В docker-деплое ключи кладутся в файл `.env` рядом с
   `docker-compose.yml` (см. пример ниже). Значения никогда не попадают
   в логи — логируется только сам факт enabled/disabled.

Если очень нужно задать ключи конфигом (модель `LiveConfig` такие поля
умеет, конфиг приоритетнее окружения) — создайте **копию** `config/live.yaml`
вне репозитория (например, `/etc/trading-bot/live.yaml`), впишите
`telegram_bot_token` / `telegram_chat_id` в неё и запускайте раннер с
`--config /etc/trading-bot/live.yaml`. В коммитимый `config/live.yaml`
секреты не вписывать — файл трекается в git.

```bash
# .env на сервере, рядом с docker-compose.yml
...
| Событие | Когда | Троттлинг |
| --- | --- | --- |
| 🟢 «Раннер запущен» | старт **постоянного** процесса (без `--once`), один раз, с позицией на момент старта; в cron-режиме `--once` не шлётся | нет |
| 📈 вход / 📉 выход | исполнение сигнальных ордеров, с ценой, объёмом и ~PnL | нет |
| 🛑 стоп-лосс / 🎯 тейк-профит | пробой уровня тикером, немедленный выход | нет |
| ⚠️ «Требуется внимание» | поднят `needs_attention` — торговля стоит | нет, один раз на переход |
| 🛑/⏸ STOP/PAUSE | переход свитча в активное/неактивное состояние | нет, только переходы |
| ⚠️ network error / ❌ unexpected error | сетевые/данные сбои и неожиданные ошибки цикла (лог + Telegram) | не чаще раза в `error_throttle_minutes` |
| 💤 Heartbeat | дайджест «позиция/equity/возраст свечи/свитчи», ежедневно в `heartbeat_time` (по МСК, формат `HH:MM`; `null` — выключить); **только в режиме постоянного процесса** — расписание живёт в памяти процесса, при cron-эксплуатации (`--once`) дайджеста нет | — |

Инвариант: **сбой отправки уведомления никогда не рвёт торговый цикл** —
неудачный запрос к Telegram логируется warning'ом (токен при этом
маскируется), а торговля продолжается. Реализация — `live/notify.py`
(stdlib urllib, без новых зависимостей).
...
src/trading_bot/
  ...
  live/            live/paper-раннер: state (персистентность), execution
                   ...
                   notify (Telegram-уведомления, stdlib urllib)
...
Для Telegram-уведомлений в docker-деплое создайте файл `.env` рядом с
`docker-compose.yml` (`TELEGRAM_BOT_TOKEN=...`, `TELEGRAM_CHAT_ID=...`) —
compose передаст их в контейнер (после создания/правки `.env` пересоздайте
