...
- ``critical`` — старт раннера (только в режиме постоянного процесса),
  ``needs_attention`` (без троттлинга);
- ``trade`` — входы/выходы, стоп/тейк по тикеру (без троттлинга);
- ``switch`` — переходы STOP/PAUSE (без троттлинга);
- ``error`` — сетевые/данные сбои и неожиданные ошибки цикла; троттлинг:
  не чаще раза в ``error_throttle_minutes`` (первые всегда уходят);
...
# Трейдофф: send вызывается синхронно из торгового цикла, поэтому каждая
# секунда таймаута = секунда задержки цикла. Telegram при живой сети отвечает
# меньше секунды; длинный таймаут при мёртвой сети ничего не спасает, а цикл
# (и следующие за ним проверки стопа по тикеру) только оттягивает.
